...
def train_pixel_seg_model(params):
    ...
    model = train_model(x_train, y_train, x_test, y_test, model, params, logger)
    for file_name in image_file_test:
        logger.info(f"Getting inference for [{file_name}]")
        x = cv2.imread(file_name)
        im_shape = x.shape
        x = get_image(x, h, w)
        y_pred = get_prediction(model, x, im_shape, h, w)
        f_name = f"output/{file_name.split('/')[-1].split('.')[0]}_pred.png"
